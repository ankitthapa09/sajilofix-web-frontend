...
function initialsFromName(fullName?: string) {
  ...
  const a = parts[0]?.[0] ?? "A";
  const b = parts.length > 1 ? parts[parts.length - 1]?.[0] ?? "U" : (parts[0]?.[1] ?? "U");
...
export default function AuthoritySettingsProfile() {
  ...
  React.useEffect(() => {
    void (async () => {
      try {
        ...
        setEmail(user.email ?? "");
        setPhone(user.phone ?? "");
        setAddress([user.municipality, user.wardNumber ? `Ward ${user.wardNumber}` : ""].filter(Boolean).join(", "));
        setDob(user.dob ?? "");
      } catch {
      } finally {
        setLoading(false);
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-end">
          <button
            ...
            className="h-10 px-5 rounded-md bg-blue-600 text-white font-semibold hover:bg-blue-700 disabled:opacity-60"
          >
    ...
    <div className={className}>
      ...
      <div className="mt-1 relative">
        ...
        <input
          ...
          className={
            "h-10 w-full rounded-md border border-gray-200 bg-gray-50 px-3 outline-none focus:bg-white focus:ring-2 focus:ring-blue-200 " +
            (icon ? "pl-9" : "")
          }
        />
      </div>
